...
def getStreamUrl(id,live=False):
    ...
    if content.get('error') is not None:
        ...
        if not live:        #['380'][0]['url']
            try:
                url1080 = cc.get('1080')[1].get('url') # 0 is x-mpeg dont work w/ kodi
                if url1080:
                    rr = requests.head(url1080,allow_redirects=True)
                    if rr.headers.get('Content-Length') == '0' :
                        raise
                    else:
                        return url1080
            except Exception:
...
def addLink(name, url, mode, iconimage, user, desc, duration, date, nr,views):
    ...
    entries.append(('Find ' + user + ' Other Videos', 'XBMC.RunPlugin(plugin://plugin.video.dailymotion_com/?mode=listVideos&url=user:'+user+')',))
    if dmUser == "":
